...
exports.addProjectController = catchAsync(async (req, res, next) => {
  ...
  const { project_details, daily_work_hour, project_workers, project_details_for_workers, client_details, project_time_economical_details } = req.body;

  if (!project_details || !daily_work_hour || !project_workers || !project_details_for_workers || !client_details || !project_time_economical_details) {
    return next(new AppError("Some Project details missing", 400));
  }
  const project = await projectMode.create(req.body);
  if (!project) {
...
exports.getAllProjectsController = catchAsync(async (req, res, next) => {
  const projects = await projectMode.find();
...
exports.getSingleProjectController = catchAsync(async (req, res, next) => {
  ...
  const project = await projectMode.findById(project_id);
  if (!project) {
...
exports.deleteProjectController = catchAsync(async (req, res, next) => {
  ...
  const project = await projectMode.findByIdAndUpdate(project_id, { is_active: false }, { new: true });
  if (!project) {
    return next(new AppError("Project not found", 400));
  }
