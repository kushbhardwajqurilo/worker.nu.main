...
const { companyModel, companyAliasModel } = require("../../models/company.mode");
const {
  catchAsync,
  AppError,
  sendSuccess,
...
exports.addCompanyController = catchAsync(async (req, res, next) => {
  ...
  const insert = await companyModel.create(req.body);
  if (!insert) {
...
exports.updateCompanyController = catchAsync(async (req, res, next) => {
  ...
  const result = await companyModel.updateOne({ _id: c_id }, req.body);
...
exports.getCompanyDetailController = catchAsync(async (req, res, next) => {
  ...
  const find = await companyModel.findOne({ _id: req.query.c_id });
...
exports.addCompanyAlias = catchAsync(async (req, res, next) => {
  ...
  const companyExists = await companyModel.exists({ _id: c_id });
  if (!companyExists) {
...
exports.getAllCompanyAliasController = catchAsync(async (req, res, next) => {
  ...
  const companyAlias = await companyAliasModel.find({isDelete:{$ne:true}});
...
exports.getSingleCompanyAliasController = catchAsync(async (req, res, next) => {
  ...
  const result = await companyAliasModel.findOne({_id:c_id,isDelete:{$ne:true}});
  if (!result) {
    return next(new AppError("no company alias found", 400))
  }
...
exports.updateCompanyAliasController = catchAsync(async (req, res, next) => {
  ...
  const result = await companyAliasModel.updateOne({ _id: c_id }, req.body);
  if (result.modifiedCount === 0) {
...
exports.deleteCompanyAliasController = catchAsync(async (req, res, next) => {
  ...
  const isAliasExist = await companyAliasModel.findOne({ _id: c_id });
  if (!isAliasExist) {
    return next(new AppError("alias not found", 400));
  }
...
exports.deleteMultipleCompanyAliasController = catchAsync(async (req, res, next) => {
  const { c_id } = req.body;
  if (!c_id || c_id.length === 0) {
    return next(new AppError("company id missing", 400));
  }
  ...
  const result = await companyAliasModel.updateMany({ _id: { $in: c_id } }, { $set: { isDelete: true } });
